...
데이터베이스에 직접 SQL 을 날리면 하나의 쿼리가 발생했겠지만, ORM 기술을 이용해 자동화된 SQL 쿼리문을 날리면서 생긴 문제다.

JPA의 경우 객체에 대해 조회한다고 해도, 다양한 연관관계들의 매핑에 의해 관계가 맺어진 다른 객체들까지 함께 조회되는 경우 N+1 문제가 발생하게 된다.
...
먼저 `즉시 로딩`에 대해 살펴보자.
즉시 로딩은 많은 강의에서 강조하지만, 실무에서 쓰지 말아야 할 모든 문제의 근원이 되는 로딩 방식이다.
❌(반드시 사용하지 말아야 하는 것은 아니다. 간단한 연관 관계의 경우 사용하는 것이 오히려 성능 면에서 좋을 때도 있다)
...
User의 수가 적으면 크게 문제가 되진 않겠지만, 만약 User의 수가 100명이고 한 유저당 10개의 게시글을 작성했다고 치면 1 + 100 = 101 개의 쿼리가 실행된다.
...
N+1 문제의 근본적인 JPA의 연관관계 처리 방식에서 비롯된다.

따라서 직접 JPQL을 작성해 연관 객체를 모두 한 번에 불러올 수 있으면 문제를 해결할 수 있을 것 같다.
...
즉시 로딩은 모든 상황에서 무조건 연관 객체를 불러오지만, fetch join은 필요할 때만 제어할 수 있어 더 유연하고 성능에도 이점이 있다

---

<h1 id=5>5️⃣ @EntityGraph</h1>

fetch join으로 N+1 문제를 해결했지만, fetch join은 하드 코딩을 해야하는 문제가 있다. 이를 최소화하고 싶으면 `@EntityGraph`를 사용하면 된다.
...
따라서 Pagination에서는 fetch join을 하고싶어서 한다고 하더라도 해결을 할 수 없다.
...
- 하이버네이트는 그냥 다 들고온 다음에 애플리케이션에서 처리하는 쪽을 선택한거다.

---
## ✅페이징 처리시 인메모리 사용 문제 해해결 방법
...
배치 사이즈는 사이즈 갯수 제한을 설정해 사용자가 최적화된 데이터 사이즈를 적용하게끔 도와준다면, `@Fetch`는 그냥 전부 다 해준다.
...
5. 2개 이상의 collection join : BatchSize 써라
